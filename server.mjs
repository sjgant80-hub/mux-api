...
createServer(async (req, res) => {
  ...
  try {
    if (req.method === 'GET' && req.url === '/health') return res.end(JSON.stringify({ status: 'ok', wraps: 'Mux' }));
    if (req.method === 'GET' && req.url === '/') return res.end(JSON.stringify({ name: 'mux-api', wraps: 'Mux', methods: Object.getOwnPropertyNames(Object.getPrototypeOf(client)).filter(n => n !== 'constructor') }));
...
}).listen(PORT, () => console.log('mux-api listening on', PORT));
